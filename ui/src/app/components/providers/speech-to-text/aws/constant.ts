 ...
 */
import { AWS_SPEECH_TO_TEXT_LANGUAGE } from '@/providers';
import { SetMetadata } from '@/utils/metadata';
import { Metadata } from '@rapidaai/react';

export const GetAWSDefaultOptions = (current: Metadata[]): Metadata[] => {
  const mtds: Metadata[] = [];
  ...
  addMetadata('listen.language', 'en-US', value =>
    AWS_SPEECH_TO_TEXT_LANGUAGE().some(l => l.code === value),
  );
...
export const ValidateAWSOptions = (
  options: Metadata[],
): string | undefined => {
  ...
  if (
    ...
    return 'Please provide a valid aws credential for speech to text.';
  }
  ...
  if (
    !languageOption ||
    !AWS_SPEECH_TO_TEXT_LANGUAGE().some(
      lang => lang.code === languageOption.getValue(),
    )
  ) {
    return 'Please provide a valid aws language for speech to text.';
  }
