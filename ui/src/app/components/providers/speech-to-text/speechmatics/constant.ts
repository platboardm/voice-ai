...
export const GetSpeechmaticsDefaultOptions = (
  current: Metadata[],
): Metadata[] => {
  const mtds: Metadata[] = [];
  ...
  addMetadata('listen.language', 'en', value =>
    SPEECHMATICS_LANGUAGE().some(l => l.language_id === value),
  );
...
export const ValidateSpeechmaticsOptions = (
  ...
  if (
    ...
    return 'Please provide a valid speechmatics credential for speech to text.';
  }
  ...
  if (
    !languageOption ||
    !SPEECHMATICS_LANGUAGE().some(
      lang => lang.language_id === languageOption.getValue(),
    )
  ) {
    return 'Please provide a valid speechmatics language for speech to text.';
  }
