...
func (n *minimaxNormalizer) Normalize(ctx context.Context, text string) string {
	if text == "" {
		return text
	}
...
func (n *minimaxNormalizer) removeMarkdown(input string) string {
...
func (n *minimaxNormalizer) normalizeWhitespace(text string) string {
	re := regexp.MustCompile(`\s+`)
