...
func (n *resembleaiNormalizer) Normalize(ctx context.Context, text string) string {
	if text == "" {
		return text
	}
...
func (n *resembleaiNormalizer) removeMarkdown(input string) string {
...
func (n *resembleaiNormalizer) normalizeWhitespace(text string) string {
	re := regexp.MustCompile(`\s+`)
