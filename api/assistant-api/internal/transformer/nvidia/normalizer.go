...
func (n *nvidiaNormalizer) Normalize(ctx context.Context, text string) string {
	if text == "" {
		return text
	}
...
func (n *nvidiaNormalizer) removeMarkdown(input string) string {
...
func (n *nvidiaNormalizer) normalizeWhitespace(text string) string {
	re := regexp.MustCompile(`\s+`)
